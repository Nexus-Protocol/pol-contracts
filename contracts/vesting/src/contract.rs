...
use crate::state::{
    ...
    update_vesting_info, Config,
};
use cw20::Cw20ExecuteMsg;
use services::vesting::{
    ClaimableAmountResponse, ConfigResponse, ExecuteMsg, InstantiateMsg, MigrateMsg, OrderBy,
    QueryMsg, VestingAccount, VestingAccountResponse, VestingAccountsResponse, VestingInfo,
    VestingSchedule,
};
...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> StdResult<Response> {
    match msg {
        ...
        ExecuteMsg::AddVestingSchedule { addr, schedule } => {
            assert_owner_privilege(deps.storage, deps.api, info.sender)?;
            add_vesting_schedule(deps, addr, schedule)
        }
    }
}
...
pub fn add_vesting_schedule(
    ...
    assert_vesting_schedules(&[schedule.clone()])?;
    let config: Config = read_config(deps.storage)?;
    let vesting_addr = deps.api.addr_canonicalize(&addr)?;
    update_vesting_info(deps.storage, &vesting_addr, |vi| match vi {
        Some(mut vi) => {
            vi.schedules.push(schedule);
            Ok(vi)
        }
        None => Ok(VestingInfo {
    ...
    Ok(Response::new().add_attribute("action", "add_vesting_schedule"))
}

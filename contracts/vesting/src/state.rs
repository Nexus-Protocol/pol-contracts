...
use services::vesting::{OrderBy, VestingInfo};
...
pub fn update_vesting_info<A>(
    storage: &mut dyn Storage,
    address: &CanonicalAddr,
    action: A,
) -> StdResult<VestingInfo>
where
    A: FnOnce(Option<VestingInfo>) -> StdResult<VestingInfo>,
{
    bucket::<VestingInfo>(storage, PREFIX_KEY_VESTING_INFO).update(address.as_slice(), action)
}
